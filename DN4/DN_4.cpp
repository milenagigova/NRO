// DN_4.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main() {
    ...
    for (int ii = 0; ii < 2000; ii++)
    {
        ...
        // Glavna zanka za iteracije po vseh elementih vektorja T
        for (int i = 0; i < n_b; i++) {
            double sum = b[i];

            // Seštejemo vse vrednosti, ki niso na diagonali
            for (int j = 0; j < n_b; j++) {
                if (i != j) {
                    sum -= A[i][j] * T[j]; 
                }
            ...
            // Izraèunamo novo vrednost za T[i] (novo temp.)
            double new_T = sum / A[i][i];

            // Preverimo, ali je vrednost spremenjena v primerjavi s prejšnjo
            if (fabs(new_T - T[i]) > 1e-6) {
                converged = false; // Ni konvergiralo, nadaljujemo z iteracijami
            }

            T[i] = new_T; 
        }

        // Èe smo dosegli konvergenco, prekinemo z iteracijami
        if (converged) {
            break;
        }
